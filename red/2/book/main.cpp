...
class LogDuration {
public:
	LogDuration(const string& h) : start(steady_clock::now()), hint(h) {
	}
	~LogDuration() {
		auto finish = steady_clock::now();
		auto dur = finish - start;
		cerr << hint << " took "<< duration_cast<milliseconds>(dur).count() << "ms" << endl;
	}
private:
...
int main() {
#ifdef TEST_BUILD  
  ifstream my_in("in.txt");
  streambuf *cinbuf = std::cin.rdbuf(); //save old buf
  cin.rdbuf(my_in.rdbuf()); //redirect std::cin to in.txt!
#endif
